...
type InboxPriority = 'critical' | 'high'

type InboxStatus = 'not_started' | 'in_progress' | 'waiting'
...
function getActionLabel(status: InboxStatus) {
  switch (status) {
    case 'not_started':
...
export default async function DashboardPage() {
    const workInboxItems = await getWorkInboxItems()
const databaseInboxItems: WorkInboxItem[] = workInboxItems.map((item) => ({
  id: item.id,
...
sourceId: item.sourceId,
    ...
    <div style={page}>
      <header style={header}>
        ...
        <div style={headerActions}>
          <button type="button" style={newButton}>
            + Novo
          </button>
      ...
      <div style={workspace}>
        <aside style={sidebar}>
          <nav style={navigation}>
            <a href="/dashboard" style={activeNavigationItem}>
              🏠 Komandni centar
            </a>
  ...
  📋 Dnevne kontrole
        ...
        <main style={main}>
          <div style={dashboardGrid}>
            <section style={prioritiesPanel}>
              <div style={sectionHeader}>
                <h2 style={sectionTitle}>Operativni prioriteti</h2>
                <p style={sectionSubtitle}>Gde danas treba da radiš</p>
              </div>

              <div style={priorityList}>
                {priorities.map((priority) => (
                  <article key={priority.id} style={priorityCard}>
                    <div style={priorityHeader}>
                      <h3 style={priorityTitle}>{priority.title}</h3>
  ...
  >
    Otvori
            ...
            <section style={inboxPanel}>
              <div style={inboxHeader}>
                <div>
                  <h2 style={sectionTitle}>Radni Inbox</h2>
                  <p style={sectionSubtitle}>Moji otvoreni zadaci</p>
                </div>

                <button type="button" style={showAllButton}>
                  Prikaži sve ({databaseInboxItems.length})
                </button>
              </div>

              <div style={inboxList}>
                {databaseInboxItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div style={inboxTopRow}>
                        <div>
                          <div style={employerName}>
                            {item.employerName}
                          </div>

                          <div style={categoryLabel}>
                            {item.category}
                          </div>
                        </div>
                      ...
                      <div style={taskTitle}>{item.title}</div>

                      <div style={subjectLabel}>{item.subject}</div>
...
const activeNavigationItem: React.CSSProperties = {
  ...navigationItem,
  background: '#eff6ff',
  color: '#1d4ed8',
  fontWeight: 700,
...
const dashboardGrid: React.CSSProperties = {
  display: 'grid',
  gridTemplateColumns: '285px minmax(0, 900px)',
  gap: 12,
  alignItems: 'start',
  justifyContent: 'start',
}

const prioritiesPanel: React.CSSProperties = {
  padding: 16,
  background: '#ffffff',
...
const inboxPanel: React.CSSProperties = {
  minWidth: 0,
  padding: 16,
  background: '#ffffff',
  border: '1px solid #e5e7eb',
...
const priorityList: React.CSSProperties = {
  display: 'flex',
  flexDirection: 'column',
  gap: 10,
}
...
const priorityHeader: React.CSSProperties = {
  display: 'flex',
  alignItems: 'center',
  justifyContent: 'space-between',
  gap: 8,
...
const priorityCount: React.CSSProperties = {
  marginTop: 8,
  color: '#4b5563',
  fontSize: 12,
}
...
const inboxList: React.CSSProperties = {
  display: 'flex',
  flexDirection: 'column',
  gap: 9,
}

const inboxCard: React.CSSProperties = {
  padding: 13,
  border: '1px solid #e5e7eb',
  borderLeftWidth: 4,
  borderRadius: 7,
...
const inboxTopRow: React.CSSProperties = {
  display: 'flex',
  alignItems: 'flex-start',
  justifyContent: 'space-between',
  gap: 14,
...
const employerName: React.CSSProperties = {
  fontSize: 12,
  fontWeight: 800,
  color: '#374151',
}
